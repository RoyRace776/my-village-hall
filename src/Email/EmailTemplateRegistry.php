    ...
    public static function all(): array {
        return [
            ...
            'booking-cancelled' => [
                ...
                'default_subject' => 'Booking cancelled: {{booking_ref}}',
                'placeholders' => [
                    'customer_name' => 'Customer full name.',
                    'customer_address' => 'Customer address (single line).',
                    'booking_ref' => 'Booking reference/ID.',
                ...
                'default_html' => '<table style="width:100%;max-width:620px;margin:auto;font-family:sans-serif;background:#ffffff;border-radius:8px;box-shadow:0 2px 8px #0001;"><tr><td style="padding:28px 32px 12px 32px;">{{logo_html}}<h2 style="margin:0 0 8px 0;color:#222;">Booking cancelled</h2><p style="margin:0 0 16px 0;color:#444;">Hi {{customer_name}}, your booking has been cancelled.</p><p style="margin:0 0 10px 0;"><strong>Reference:</strong> {{booking_ref}}<br><strong>Date:</strong> {{booking_date}}<br><strong>Time:</strong> {{booking_time}}<br><strong>Venue/Room:</strong> {{venue_name}} / {{room_name}}<br><strong>Amount:</strong> {{booking_amount}}</p><p style="margin:16px 0 0 0;color:#666;">If you need more information, please contact us.</p></td></tr><tr><td style="padding:0 32px 24px 32px;color:#999;font-size:12px;">{{site_name}} • {{site_url}}</td></tr></table>',
                'sample_vars' => [
                    'customer_name' => 'Alex Morgan',
                    'customer_address' => '1 High Street, Exampletown',
                    'booking_ref' => '#BK-1042',
            ...
            'payment-receipt' => [
                ...
                'description' => 'Sent when a payment is recorded and a receipt is requested.',
                'default_subject' => 'Payment receipt for invoice {{invoice_ref}}',
                'placeholders' => [
                    'customer_name' => 'Customer full name.',
                    'customer_address' => 'Customer address (single line).',
                    'invoice_ref' => 'Invoice reference/number.',
                    ...
                    'invoice_status' => 'Current invoice status.',
                    'organisation_name' => 'Organisation linked to the invoice.',
                    'payment_id' => 'Payment numeric ID.',
                    'payment_amount' => 'Payment amount.',
                    'payment_date' => 'Payment date.',
                    'payment_method' => 'Payment method label.',
                    'payment_reference' => 'Payment reference/transaction reference.',
                    'payment_comment' => 'Payment comment/notes.',
                    'amount_due' => 'Remaining balance after this payment.',
                    'site_name' => 'Current site name.',
                    'site_url' => 'Current site URL.',
                    'logo_url' => 'Current site icon URL.',
                ],
                'default_html' => '<table style="width:100%;max-width:620px;margin:auto;font-family:sans-serif;background:#ffffff;border-radius:8px;box-shadow:0 2px 8px #0001;"><tr><td style="padding:28px 32px 12px 32px;">{{logo_html}}<h2 style="margin:0 0 8px 0;color:#222;">Payment receipt</h2><p style="margin:0 0 14px 0;color:#444;">Hi {{customer_name}}, we have received your payment.</p><p style="margin:0 0 10px 0;"><strong>Invoice:</strong> {{invoice_ref}}<br><strong>Payment amount:</strong> {{payment_amount}}<br><strong>Payment date:</strong> {{payment_date}}<br><strong>Method:</strong> {{payment_method}}<br><strong>Reference:</strong> {{payment_reference}}</p><p style="margin:16px 0 10px 0;color:#444;"><strong>Remaining balance:</strong> {{amount_due}}</p><p style="margin:0;color:#666;">Thank you for your payment.</p></td></tr><tr><td style="padding:0 32px 24px 32px;color:#999;font-size:12px;">{{site_name}} • {{site_url}}</td></tr></table>',
                'sample_vars' => [
                    'customer_name' => 'Alex Morgan',
                    'customer_address' => '1 High Street, Exampletown',
                    'invoice_ref' => 'INV-2026-041',
                    ...
                    'payment_comment' => 'Deposit received',
                    'amount_due' => '130.00',
                ],
            ],
