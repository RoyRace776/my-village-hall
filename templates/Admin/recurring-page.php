...
if ($edit_id || $view_id) {
    ...
    $pattern      = $myvh_container->get(RecurringPatternService::class)->get($pattern_id);

    if (!$pattern) {
        wp_die(__('Pattern not found.', 'my-village-hall'));
    }

    $parent_booking  = $myvh_container->get(BookingService::class)->get_by_id($pattern['ParentBookingId']);
    $bookings        = $myvh_container->get(RecurringPatternService::class)->get_bookings_for_pattern($pattern_id);
                    ...
                    <?php if ($is_view_mode): ?>
                        ...
                        <table class="form-table">
                            <tr><th><?php _e('Schedule', 'my-village-hall'); ?></th>
                                <td><strong><?php echo esc_html(RecurringPatternService::describe($pattern)); ?></strong></td></tr>
...
// ── LIST VIEW ─────────────────────────────────────────────────────────────────
$patterns = $myvh_container->get(RecurringPatternService::class)->get_active_with_bookings() ?? [];
?>
<div class="wrap">
